...
# create_message принимает message: str как параметр запроса (query parameter).
...
# И все данные в параметре запроса видны в URL,
# что не соответствует лучшим практикам для POST-запросов, так как снижает безопасность и ограничивает размер данных.


# Чтобы отправлять данные в теле запроса, используем класс Body из FastAPI.
# Это позволяет извлечь данные из тела POST-запроса, делая его более безопасным и гибким
@app.post("/messages", status_code=status.HTTP_201_CREATED)
...
# uvicorn crud_3_1:app --port 8080 --reload
